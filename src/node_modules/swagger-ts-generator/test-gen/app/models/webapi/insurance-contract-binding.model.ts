...
export interface IInsuranceContractBinding {
...
export class InsuranceContractBinding extends BaseModel implements IInsuranceContractBinding  {
    static TYPE_SELECTOR = 'InsuranceContractBinding';
    ...
    */
    constructor(values?: Partial<IInsuranceContractBinding>) {
        super();

        this.typeSelector = InsuranceContractBinding.TYPE_SELECTOR;
    ...
    */
    setValues(values: Partial<IInsuranceContractBinding>): void {
        if (values) {
            
            const rawValues = values
            this.contract.setValues(rawValues.contract);
            this.createdAt = this.getValue<Date>(rawValues, InsuranceContractBinding.CREATED_AT_FIELD_NAME);
            this.description = this.getValue<string>(rawValues, InsuranceContractBinding.DESCRIPTION_FIELD_NAME);
            this.followUpDueAt = this.getValue<Date>(rawValues, InsuranceContractBinding.FOLLOW_UP_DUE_AT_FIELD_NAME);
            this.id = this.getValue<string>(rawValues, InsuranceContractBinding.ID_FIELD_NAME);
            ...
            this.fillModelArray<IBureauLeader | ICedent | ICoAgreementParty | IContractHolder | ICoverHolder | IFinancialBroker | IInsuranceBroker | IInsured | IPricingLeader | IPrimaryInsurer | IProducingBroker | IProducingContractHolder | IProducingInsurer | IReinsurer | ISlipLeader>(this, InsuranceContractBinding.PARTICIPANTS_FIELD_NAME, rawValues.participants, BureauLeader, SubTypeFactory.createSubTypeInstance);
            this.fillModelArray<IStateChangeRecord>(this, InsuranceContractBinding.STATE_CHANGE_RECORDS_FIELD_NAME, rawValues.stateChangeRecords, StateChangeRecord, SubTypeFactory.createSubTypeInstance);
            this.status = this.getValue<InsuranceContractProcessStatus>(rawValues, InsuranceContractBinding.STATUS_FIELD_NAME);
            this.typeSelector = this.getValue<string>(rawValues, InsuranceContractBinding.TYPE_SELECTOR_FIELD_NAME);
        }
    }
}

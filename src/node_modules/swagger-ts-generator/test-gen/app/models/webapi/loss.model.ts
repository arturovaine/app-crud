...
export class Loss extends BaseModel implements ILoss  {
    ...
    */
    constructor(values?: Partial<ILoss>) {
        super();
    ...
    */
    setValues(values: Partial<ILoss>): void {
        if (values) {
            
            const rawValues = values
            this.asOf = this.getValue<Date>(rawValues, Loss.AS_OF_FIELD_NAME);
            this.currency = this.getValue<string>(rawValues, Loss.CURRENCY_FIELD_NAME);
            this.fillModelArray<IDocument>(this, Loss.DOCUMENTS_FIELD_NAME, rawValues.documents, Document, SubTypeFactory.createSubTypeInstance);
            this.fillModelArray<IGrossNetAmount>(this, Loss.GROSS_NET_AMOUNTS_FIELD_NAME, rawValues.grossNetAmounts, GrossNetAmount, SubTypeFactory.createSubTypeInstance);
            this.id = this.getValue<string>(rawValues, Loss.ID_FIELD_NAME);
            ...
            this.name = this.getValue<string>(rawValues, Loss.NAME_FIELD_NAME);
            this.shared = this.getValue<boolean>(rawValues, Loss.SHARED_FIELD_NAME);
            this.status = this.getValue<LossStatus>(rawValues, Loss.STATUS_FIELD_NAME);
            this.transactionStatus = this.getValue<TransactionStatus>(rawValues, Loss.TRANSACTION_STATUS_FIELD_NAME);
            this.version = this.getValue<number>(rawValues, Loss.VERSION_FIELD_NAME);
            this.versionCreatedAt = this.getValue<Date>(rawValues, Loss.VERSION_CREATED_AT_FIELD_NAME);
        }
    }
}

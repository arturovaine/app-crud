...
export interface IInsuranceContractQuotation {
...
export class InsuranceContractQuotation extends BaseModel implements IInsuranceContractQuotation  {
    static TYPE_SELECTOR = 'InsuranceContractQuotation';
    ...
    */
    constructor(values?: Partial<IInsuranceContractQuotation>) {
        super();

        this.typeSelector = InsuranceContractQuotation.TYPE_SELECTOR;
    ...
    */
    setValues(values: Partial<IInsuranceContractQuotation>): void {
        if (values) {
            
            const rawValues = values
            this.contract.setValues(rawValues.contract);
            this.createdAt = this.getValue<Date>(rawValues, InsuranceContractQuotation.CREATED_AT_FIELD_NAME);
            this.description = this.getValue<string>(rawValues, InsuranceContractQuotation.DESCRIPTION_FIELD_NAME);
            this.followUpDueAt = this.getValue<Date>(rawValues, InsuranceContractQuotation.FOLLOW_UP_DUE_AT_FIELD_NAME);
            this.id = this.getValue<string>(rawValues, InsuranceContractQuotation.ID_FIELD_NAME);
            ...
            this.fillModelArray<IBureauLeader | ICedent | ICoAgreementParty | IContractHolder | ICoverHolder | IFinancialBroker | IInsuranceBroker | IInsured | IPricingLeader | IPrimaryInsurer | IProducingBroker | IProducingContractHolder | IProducingInsurer | IReinsurer | ISlipLeader>(this, InsuranceContractQuotation.PARTICIPANTS_FIELD_NAME, rawValues.participants, BureauLeader, SubTypeFactory.createSubTypeInstance);
            this.fillModelArray<IStateChangeRecord>(this, InsuranceContractQuotation.STATE_CHANGE_RECORDS_FIELD_NAME, rawValues.stateChangeRecords, StateChangeRecord, SubTypeFactory.createSubTypeInstance);
            this.status = this.getValue<InsuranceContractProcessStatus>(rawValues, InsuranceContractQuotation.STATUS_FIELD_NAME);
            this.typeSelector = this.getValue<string>(rawValues, InsuranceContractQuotation.TYPE_SELECTOR_FIELD_NAME);
        }
    }
}

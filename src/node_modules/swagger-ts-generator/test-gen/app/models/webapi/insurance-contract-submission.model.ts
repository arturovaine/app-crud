...
export interface IInsuranceContractSubmission {
...
export class InsuranceContractSubmission extends BaseModel implements IInsuranceContractSubmission  {
    static TYPE_SELECTOR = 'InsuranceContractSubmission';
    ...
    */
    constructor(values?: Partial<IInsuranceContractSubmission>) {
        super();

        this.typeSelector = InsuranceContractSubmission.TYPE_SELECTOR;
    ...
    */
    setValues(values: Partial<IInsuranceContractSubmission>): void {
        if (values) {
            
            const rawValues = values
            this.contract.setValues(rawValues.contract);
            this.createdAt = this.getValue<Date>(rawValues, InsuranceContractSubmission.CREATED_AT_FIELD_NAME);
            this.description = this.getValue<string>(rawValues, InsuranceContractSubmission.DESCRIPTION_FIELD_NAME);
            this.followUpDueAt = this.getValue<Date>(rawValues, InsuranceContractSubmission.FOLLOW_UP_DUE_AT_FIELD_NAME);
            this.id = this.getValue<string>(rawValues, InsuranceContractSubmission.ID_FIELD_NAME);
            ...
            this.fillModelArray<IBureauLeader | ICedent | ICoAgreementParty | IContractHolder | ICoverHolder | IFinancialBroker | IInsuranceBroker | IInsured | IPricingLeader | IPrimaryInsurer | IProducingBroker | IProducingContractHolder | IProducingInsurer | IReinsurer | ISlipLeader>(this, InsuranceContractSubmission.PARTICIPANTS_FIELD_NAME, rawValues.participants, BureauLeader, SubTypeFactory.createSubTypeInstance);
            this.fillModelArray<IStateChangeRecord>(this, InsuranceContractSubmission.STATE_CHANGE_RECORDS_FIELD_NAME, rawValues.stateChangeRecords, StateChangeRecord, SubTypeFactory.createSubTypeInstance);
            this.status = this.getValue<InsuranceContractProcessStatus>(rawValues, InsuranceContractSubmission.STATUS_FIELD_NAME);
            this.typeSelector = this.getValue<string>(rawValues, InsuranceContractSubmission.TYPE_SELECTOR_FIELD_NAME);
        }
    }
}
